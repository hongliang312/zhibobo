package com.example.myyuekaolianxiti.adapter;
...
import com.example.myyuekaolianxiti.R;
import com.example.myyuekaolianxiti.bean.CartBean;
import com.example.myyuekaolianxiti.bean.MessageEvent;
import com.example.myyuekaolianxiti.bean.PriceAndCountEvent;
import com.example.myyuekaolianxiti.bean.SomeId;
import com.example.myyuekaolianxiti.view.AddDeleteView;
...
public class ShopCartExListAdapter extends BaseExpandableListAdapter {
    ...
    public ShopCartExListAdapter(Context context, List<CartBean.DataBean> list) {
...
//            holder.et_num = (EditText) view.findViewById(R.id.et_num);
        ...
        final CartBean.DataBean.ListBean datasBean = list.get(groupPosition).getList().get(childPosition);
...
//                int num = datasBean.getNum();
        ...
        //编辑
        //判断是否在编辑状态下(标题中的编辑)
        if (list.get(groupPosition).isEditor() == true) {
    ...
    class GroupViewClick implements View.OnClickListener {
        ...
        private CartBean.DataBean group;

        public GroupViewClick(int groupPosition, TextView edtor, CartBean.DataBean group) {
    ...
    private PriceAndCountEvent compute() {
        ...
        int price = 0;
        for (int i = 0; i < list.size(); i++) {
            List<CartBean.DataBean.ListBean> datasBeen = list.get(i).getList();
